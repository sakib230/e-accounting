...
<div class="col-md-12 col-sm-12 col-xs-12 p-t-20">
    <form id="invoiceForm" class="form-horizontal" action="<?php echo base_url() ?>Sales/editInvoice" method="post">
        ...
        <div class="row">
            <div class="form-group">
                ...
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <div class="input-group">
                        <input type="text" id="customerDetails" class="form-control" readonly value="<?php echo $invoiceInfo[0]['contact_name'] . '(' . $invoiceInfo[0]['customer'] . ')' ?>">
            ...
            <div class="form-group">
                ...
                <div class="col-md-4 col-sm-6 col-xs-12">
                    <input type="text" name="referenceNo" id="referenceNo" value="<?php echo $invoiceInfo[0]['display_reference_no'] ?>" class="form-control">
                </div>
            </div>

            <div class="form-group">
                <label class="control-label col-md-2 col-sm-3 col-xs-12" >Invoice Date <span class="danger">*</span></label>
                <div class="col-md-4 col-sm-6 col-xs-12">
                    <div class="input-group dateTxt">
                        <input type="text" name="invoiceDate" id="invoiceDate" value="<?php echo $invoiceInfo[0]['invoice_date'] ?>"  class="form-control">
            ...
            <div class="form-group">
                ...
                <div class="col-md-4 col-sm-6 col-xs-12">
                    <div class="input-group dateTxt">
                        <input type="text" name="dueDate" id="dueDate" value="<?php echo $invoiceInfo[0]['due_date'] ?>"  class="form-control">
                        <span class="input-group-addon">
                            <span class="glyphicon glyphicon-calendar"></span>
                        </span>
                    </div>
        ...
        <div class="row">
            <div class="table-custom-responsive">
                <table id="itemDetailsTable" class="table table-bordered custom-table">
                    ...
                    <?php
                    $applyItemCount = 1;
                    foreach ($invoiceItemDetails as $invoiceItem) {
                        echo '<tr id="itemDetailsTr' . $applyItemCount . '">
                                    <td class="td-left">' . $invoiceItem['item_title'] . '</td>
                                    <td><input class="form-control form-control-td-center" onchange="allCalculation()" onkeyup="allCalculation()" id="itemQuantity' . $applyItemCount . '" name="itemQuantity' . $applyItemCount . '"  type="number" min="0" value="' . $invoiceItem['quantity'] . '"></td>
                                    <td>' . $invoiceItem['unit'] . '</td>
                                    <td><input class="form-control form-control-td-right" onchange="allCalculation()" onkeyup="allCalculation()" id="itemRate' . $applyItemCount . '" name="itemRate' . $applyItemCount . '" type="number" value="' . $invoiceItem['rate'] . '"></td>
                                    <td>' . $invoiceItem['tax_title'] . '</td>
                                    <td id="amountTd' . $applyItemCount . '">' . $invoiceItem['amount'] . '</td>
                                    <td class="text-danger"><i class="fa fa-remove pointer" onclick="removeApplyItem(' . $applyItemCount . ')"></i></td>
                                    <input type="hidden" id="itemCode' . $applyItemCount . '" name="itemCode' . $applyItemCount . '"  value="' . $invoiceItem['item'] . '">
                                    <input type="hidden" id="itemTaxRate' . $applyItemCount . '" name="itemTaxRate' . $applyItemCount . '"  value="' . $invoiceItem['tax_rate'] . '">
                                    <input type="hidden" id="itemTaxCode' . $applyItemCount . '" name="itemTaxCode' . $applyItemCount . '"  value="' . $invoiceItem['tax_code'] . '">
                                    <input type="hidden" id="itemTaxTitle' . $applyItemCount . '" name="itemTaxTitle' . $applyItemCount . '"  value="' . $invoiceItem['tax_title'] . '">
                                    <input type="hidden" id="itemTitle' . $applyItemCount . '" name="itemTitle' . $applyItemCount . '"  value="' . $invoiceItem['item_title'] . '">
                                    <input type="hidden" id="itemUnitName' . $applyItemCount . '" name="itemUnitName' . $applyItemCount . '"  value="' . $invoiceItem['unit'] . '">
                                  </tr>';
                        $applyItemCount++;
                    }
                    ?>
        ...
        <div class="row">
            <div class="col-md-5 col-sm-5 col-xs-12 col-md-offset-7 col-sm-offset-7">
                ...
                <span class="float-right" id="subTotal"><?php echo $invoiceInfo[0]['sub_total'] ?></span>
                <hr>
                <span><b>Adjustment (BDT)</b></span>
                <span class="float-right m-t--10"><input type="text" class="form-control text-right" onchange="allCalculation()" onkeyup="allCalculation()" id="adjust" name="adjust" value="<?php echo $invoiceInfo[0]['adjustment'] ?>"></span>
                <hr>
                <span><b>Total (BDT)</b></span>
                <span class="float-right" id="total"><?php echo $invoiceInfo[0]['total'] ?></span>
            </div>
        </div>
        <div class="row">
            <hr>
            ...
            <div class="" >
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <div class="form-group">
                        ...
                        <textarea class="form-control" name="customerNotes" rows="5" style="resize: none"><?php echo $invoiceInfo[0]['customer_notes'] ?></textarea>
                    </div>
                </div>
                <div class="col-md-6 col-sm-6 col-xs-12">
                    <div class="form-group">
                        <label class="control-label"> Terms & Condition</label>
                        <textarea class="form-control" rows="5" name="termsCondition" style="resize: none"><?php echo $invoiceInfo[0]['terms_condition'] ?></textarea>
                    </div>
                </div>
            </div>
        </div>
        <div class="row">
            ...
            <div class="form-group">
                <div class="col-md-12 col-sm-12 col-xs-12">
                    <button type="button" class="btn btn-success btn-sm" onclick="editInvoice()"><i class="fa fa-check"></i> Save</button>
                    <a href="<?php echo base_url() ?>Sales/showInvoiceDetails?invoice=<?php echo $invoiceInfo[0]['invoice_code'] ?>" class="btn btn-danger btn-sm"><i class="fa fa-remove"></i> Cancel</a>
                </div>
            </div>
        </div>
        <input type="hidden" id="customerCode" name="customerCode" value="<?php echo $invoiceInfo[0]['customer'] ?>">
        <input type="hidden" id="applyItemCount" name="applyItemCount" value="<?php echo $applyItemCount ?>">
        <input type="hidden" id="invoiceCode" name="invoiceCode" value="<?php echo $invoiceInfo[0]['invoice_code'] ?>">
    </form>
</div>
...
<script>
    ...
    var applyItemCount = '<?php echo count($invoiceItemDetails) ?>';
    applyItemCount++;
    $(document).ready(function () {
        ...
        var itemTable = $('#item-datatable').DataTable({
            "bDestroy": true,
            "ajax": '<?php echo base_url() ?>Sales/getItem',
            "deferRender": true,
            "paging": true,
            "dom": "<'row'<'col-sm-12 col-md-12 col-xs-12'l><'col-sm-12 col-md-12 col-xs-12'f>><'row'<'col-sm-12'tr>><'row'<'col-sm-12'i><'col-sm-12'p>>",
            "aaSorting": [],
...
//        var itemDetails = new Array();
    ...
    function editInvoice() {
        ...
        if (customerCode === "" || invoiceDate === "" || dueDate === "") {
            sweetAlert('Customer, Invoice Date and Due Date are required');
            return false;
        }
        var itemCode = "";
        var flag = 0;
        ...
        swal({
            title: "Do you want to edit this invoice?",
            text: "",
            type: "warning",
