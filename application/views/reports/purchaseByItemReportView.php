    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12">
            <div class="text-center">
                ...
                <span class="font-20">Purchase By Item</span><br>
    ...
    <div class="row">
        <div class="col-md-12 col-sm-12 col-xs-12 m-t-30">
            <table class="table custom-table">
                <thead>
                    <tr>
                        <th class='td-left'>Item Name</th>
                        <th>Item Type</th>
                        <th class='td-right'>Current Purchase Price<br><small><i>BDT</i></small></th>
                        <th class='td-right'>Quantity Bought</th>
                        <th class='td-right'>Amount<br><small><i>BDT</i></small></th>
                        <th class='td-right'>Average Price<br><small><i>BDT</i></small></th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    ...
                    $totalPrice = 0;
                    foreach ($purchaseItemDetails as $purchaseItemDetail) {
                        echo "<tr>";
                        echo "<td class='td-left'>$purchaseItemDetail[item_title]</td>";
                        echo "<td>" . ucfirst($purchaseItemDetail['item_type']) . "</td>";
                        echo "<td class='td-right'>$purchaseItemDetail[purchase_rate]</td>";
                        echo "<td class='td-right'>$purchaseItemDetail[quantity_bought]</td>";
                        echo "<td class='td-right'>" . number_format($purchaseItemDetail['amount'], 2) . "</td>";
                        $avaragePrice = $purchaseItemDetail['amount'] / $purchaseItemDetail['quantity_bought'];
                        echo "<td class='td-right'>" . number_format($avaragePrice, 2) . "</td>";
                        echo "</tr>";
                        $totalQuantityBought = $totalQuantityBought + $purchaseItemDetail['quantity_bought'];
                        $totalPrice = $totalPrice + $purchaseItemDetail['amount'];
                    }

                    if (!$purchaseItemDetails) {
                        echo "<tr>";
                        ...
                        echo "<td class='td-right' colspan='3'><b>" . number_format($totalQuantityBought, 2) . "</b></td>";
    ...
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <div class="row">
                    <form id="customizeForm" action="<?php echo base_url() ?>Reports/purchaseByItem" method="post">
...
<div class="modal fade" id="item-modal" tabindex="-1" role="dialog" aria-hidden="true">
    <div class="modal-dialog modal-sm">
        <div class="modal-content">
            <div class="modal-header" style="background-color: #359db5!important">
                <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">×</span></button>
                <h4 class="modal-title" id="myModalLabel3">Item</h4>
            </div>
            <div class="modal-body">
                <div class="table-custom-responsive">
                    <table class="table table-hover custom-table" id="item-datatable">
                        <thead class="hidden">
                            <tr>
                                <th></th>
                                <th></th>
...
<script>
    ...
    $(document).ready(function () {
        ...
        var itemTable = $('#item-datatable').DataTable({
            "bDestroy": true,
            "ajax": '<?php echo base_url() ?>Reports/getPurchaseItem',
            "deferRender": true,
            "paging": true,
            "dom": "<'row'<'col-sm-12 col-md-12 col-xs-12'l><'col-sm-12 col-md-12 col-xs-12'f>><'row'<'col-sm-12'tr>><'row'<'col-sm-12'i><'col-sm-12'p>>",
            "aaSorting": [],
    ...
    function  setItem(itemCode, itemTitle) {
        var filterTypeSerial = $('#filterTypeSerialHidden').val();
        $('#itemCode' + filterTypeSerial).val(itemCode);
        $('#itemDetails' + filterTypeSerial).val(itemTitle);
        $('#item-modal').modal('hide');
    }

    function showItem(filterTypeSerial) {
    ...
    function setFilterType(filterTypeSerial) {
        ...
        if (filterType === '1') {
            elementHtml = '<div class="input-group">\n\
                                <input type="text" id="itemDetails' + filterTypeSerial + '" placeholder="Item" class="form-control" readonly>\n\
                                <input type="hidden" id="itemCode' + filterTypeSerial + '" name="itemCode' + filterTypeSerial + '">\n\
                                <span class="input-group-btn">\n\
                                    <button type="button" class="btn btn-warning" onclick="showItem(' + filterTypeSerial + ')"><i class="fa fa-search"></i></button>\n\
                                </span>\n\
                            </div>';
